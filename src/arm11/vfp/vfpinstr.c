                   ...
                   int DYNCOM_TAG(vfpinstr)(cpu_t *cpu, addr_t pc, uint32_t instr, tag_t *tag, addr_t *new_pc, addr_t *next_pc)
{
    ...
    //DEBUG("\t\tin %s instruction is not implemented.\n", __FUNCTION__);
...
int DYNCOM_TRANS(vfpinstr)(cpu_t *cpu, uint32_t instr, BasicBlock *bb, addr_t pc)
{
    DEBUG("\t\tin %s VMLS instruction is executed out of here.\n", __FUNCTION__);
    //arch_arm_undef(cpu, bb, instr);
    int m;
    int n;
    int d ;
...
int DYNCOM_TRANS(vfpinstr)(cpu_t *cpu, uint32_t instr, BasicBlock *bb, addr_t pc)
{
    DEBUG("\t\tin %s VNMLA instruction is executed out of here.\n", __FUNCTION__);
    //arch_arm_undef(cpu, bb, instr);
    int m;
    int n;
    int d ;
...
int DYNCOM_TRANS(vfpinstr)(cpu_t *cpu, uint32_t instr, BasicBlock *bb, addr_t pc)
{
    DEBUG("\t\tin %s instruction will implement out of JIT.\n", __FUNCTION__);
    //arch_arm_undef(cpu, bb, instr);
    int m;
    int n;
    int d ;
...
int DYNCOM_TRANS(vfpinstr)(cpu_t *cpu, uint32_t instr, BasicBlock *bb, addr_t pc)
{
    DEBUG("\t\tin %s instr=0x%x, instruction is executed out of JIT.\n", __FUNCTION__, instr);
    //arch_arm_undef(cpu, bb, instr);
    int m;
    int n;
    int d ;
                   ...
                   int DYNCOM_TAG(vfpinstr)(cpu_t *cpu, addr_t pc, uint32_t instr, tag_t *tag, addr_t *new_pc, addr_t *next_pc)
{
    int instr_size = INSTR_SIZE;
    arm_tag_continue(cpu, pc, instr, tag, new_pc, next_pc);
    DEBUG("In %s, pc=0x%x, next_pc=0x%x\n", __FUNCTION__, pc, *next_pc);
    if(instr >> 28 != 0xe)
        *tag |= TAG_CONDITIONAL;
...
int DYNCOM_TRANS(vfpinstr)(cpu_t *cpu, uint32_t instr, BasicBlock *bb, addr_t pc)
{
    DEBUG("\t\tin %s instruction is executed out of JIT.\n", __FUNCTION__);
    //arch_arm_undef(cpu, bb, instr);
    int dp_op = (BIT(8) == 1);
    int d = dp_op ? BITS(12,15) | BIT(22) << 4 : BIT(22) | BITS(12,15) << 1;
    int m = dp_op ? BITS(0,3) | BIT(5) << 4 : BIT(5) | BITS(0,3) << 1;
...
int DYNCOM_TRANS(vfpinstr)(cpu_t *cpu, uint32_t instr, BasicBlock *bb, addr_t pc)
{
    DEBUG("\t\tin %s instruction will executed out of JIT.\n", __FUNCTION__);
    //arch_arm_undef(cpu, bb, instr);
    int dp_op = (BIT(8) == 1);
    int d = dp_op ? BITS(12,15) | BIT(22) << 4 : BIT(22) | BITS(12,15) << 1;
    //int m = dp_op ? BITS(0,3) | BIT(5) << 4 : BIT(5) | BITS(0,3) << 1;
...
int DYNCOM_TRANS(vfpinstr)(cpu_t *cpu, uint32_t instr, BasicBlock *bb, addr_t pc)
{
    DEBUG("\t\tin %s instruction is executed out.\n", __FUNCTION__);
    //arch_arm_undef(cpu, bb, instr);
    int dp_op = (BIT(8) == 1);
                   ...
                   int DYNCOM_TAG(vfpinstr)(cpu_t *cpu, addr_t pc, uint32_t instr, tag_t *tag, addr_t *new_pc, addr_t *next_pc)
{
    int instr_size = INSTR_SIZE;
    //DEBUG("\t\tin %s instruction is not implemented.\n", __FUNCTION__);
    DEBUG("\t\tin %s, instruction will be executed out of JIT.\n", __FUNCTION__);
    //arm_tag_trap(cpu, pc, instr, tag, new_pc, next_pc);
    arm_tag_continue(cpu, pc, instr, tag, new_pc, next_pc);
    return instr_size;
}
...
#endif
#ifdef VFP_MCR_IMPL
void VMSR(ARMul_State * state, u32 reg, u32 Rt)
{
    if (reg == 1) {
        DEBUG("VMSR :\tfpscr <= r%d=[%x]\n", Rt, state->Reg[Rt]);
        state->VFP[VFP_OFFSET(VFP_FPSCR)] = state->Reg[Rt];
    } else if (reg == 8) {
        DEBUG("VMSR :\tfpexc <= r%d=[%x]\n", Rt, state->Reg[Rt]);
...
VFPLABEL_INST:
{
    ...
    if ((inst_base->cond == 0xe) || CondPassed(cpu, inst_base->cond)) {
        ...
        if (inst_cream->reg == 1) { /* FPSCR */
            ...
            case 6:
                /* MVFR1, VFPv3 only ? */
                DEBUG("\tr%d <= MVFR1 unimplemented\n", inst_cream->Rt);
                break;
            case 7:
                /* MVFR0, VFPv3 only? */
                DEBUG("\tr%d <= MVFR0 unimplemented\n", inst_cream->Rt);
                break;
            case 8:
...
#endif
#ifdef VFP_MRC_IMPL
void VMRS(ARMul_State * state, u32 reg, u32 Rt, u32 * value)
{
    DEBUG("VMRS :");
    if (reg == 1) {
        if (Rt != 15) {
...
#endif
#ifdef VFP_STC_IMPL
int VSTR(ARMul_State * state, int type, u32 instr, u32 * value)
{
    static int i = 0;
...
#endif
#ifdef VFP_STC_IMPL
int VPUSH(ARMul_State * state, int type, u32 instr, u32 * value)
{
    static int i = 0;
...
#endif
#ifdef VFP_STC_IMPL
int VSTM(ARMul_State * state, int type, u32 instr, u32 * value)
{
    static int i = 0;
    static int single_regs, add, wback, d, n, imm32, regs;
    if (type == ARMul_FIRST) {
...
#endif
#ifdef VFP_LDC_IMPL
int VPOP(ARMul_State * state, int type, u32 instr, u32 value)
{
    static int i = 0;
    static int single_regs, add, wback, d, n, imm32, regs;
    if (type == ARMul_FIRST) {
...
#endif
#ifdef VFP_LDC_IMPL
int VLDR(ARMul_State * state, int type, u32 instr, u32 value)
{
    static int i = 0;
    static int single_reg, add, d, n, imm32, regs;
    if (type == ARMul_FIRST) {
...
#endif
#ifdef VFP_LDC_IMPL
int VLDM(ARMul_State * state, int type, u32 instr, u32 value)
{
    static int i = 0;
    static int single_regs, add, wback, d, n, imm32, regs;
    if (type == ARMul_FIRST) {
...
#define VFP_DEBUG_TRANSLATE DEBUG("in func %s, %x\n", __FUNCTION__, inst);
