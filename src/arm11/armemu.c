            ...
            case 0x02:	/* EOR reg and MLA */
                ...
                case 0x68:	/* Store Word, No WriteBack, Post Inc, Reg.  */
                    ...
                    if ((instr & 0x70) == 0x10) { //pkhbt
                        ...
                        ARMword val;
                        if(imm5 >= 32)
                            val = ((ARMword)(state->Reg[rm] >> 31L));
                        else
                            val = ((ARMword)(state->Reg[rm] >> imm5));

                        state->Reg[rd] = (val & 0xFFFF) | ((state->Reg[rn]) & 0xFFFF0000);
                        break;
                    } else if (BIT (4)) {
#ifdef MODE32
                        if (state->is_v6
    ...
    handle_v6_insn (ARMul_State * state, ARMword instr) {
        ...
        case 0x78:
            if (BITS(20, 24) == 0x18)
            {
                ...
                const u8 diff1 = (u8)labs((rn_val & 0xFF) - (rm_val & 0xFF));
                const u8 diff2 = (u8)labs(((rn_val >> 8) & 0xFF) - ((rm_val >> 8) & 0xFF));
                const u8 diff3 = (u8)labs(((rn_val >> 16) & 0xFF) - ((rm_val >> 16) & 0xFF));
                const u8 diff4 = (u8)labs(((rn_val >> 24) & 0xFF) - ((rm_val >> 24) & 0xFF));
